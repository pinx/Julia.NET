...
namespace Julia.NET.Statics
{
    ...
    {
        private void GenerateType(JuliaStaticModule mod,
                                  Any name,
                                  JType type)
        {
            try
            {
                ...
                switch (typetype)
                {
                    ...
                    case JTypeType.Abstract:
                        ...
                        DecorateWithDefAttribute(tb, JAbstractTypeAttribute.DEF_CON);
                        break;
                    case JTypeType.Primitive:
                        tb = mb.InternalBuilder.DefineNestedType(typename,
                            TypeAttributes.NestedPublic | TypeAttributes.SequentialLayout,
                            typeof(ValueType), type.SizeOf);
                        DecorateWithDefAttribute(tb, JPrimitiveAttribute.DEF_CON);
                        break;
                    default:
                        ...
                        tb = mb.InternalBuilder.DefineNestedType(typename,
                            TypeAttributes.NestedPublic | TypeAttributes.SequentialLayout,
                            typeof(ValueType));
                        DecorateWithDefAttribute(tb, JStructAttribute.DEF_CON);
                        break;
                }
            }
            catch (Exception)
        ...
        private bool ResolveLocalType(JuliaStaticModule mod,
                                      JType ty,
                                      out Type t)
        {
        ...
        private Type ResolveType(JuliaStaticModule mod,
                                 Any shortName)
        {
