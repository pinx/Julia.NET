...
namespace JuliaNET.Statics
{
    internal class JuliaStaticModule
    {
        ...
        public JuliaStaticModule(Any module,
                                 ILTypeBuilder moduleBuilder,
                                 FieldBuilder parentModuleField)
        {
    ...
    {
        private ILTypeBuilder GenerateModule(JuliaStaticModule mod,
                                             bool isFirst)
        {
            try
            {
                ...
                foreach (var name in JPrimitive.namesF.Invoke(mod.Module))
                {
                    ...
                    {
                        var val = Core.Julia.GetGlobal(mod.Module, name);
                        if (val.Is(JPrimitive.FunctionT))
                            GenerateFunction(mod, name, val);
                        else if (val.Is(JPrimitive.ModuleT) && val != mod.Module)
                        {
                            var mb = mod.ModuleBuilder.InternalBuilder.DefineNestedType(ConvertJuliaName(name.ToString()),
                                TypeAttributes.Class | TypeAttributes.NestedPublic | TypeAttributes.Abstract | TypeAttributes.Sealed);
